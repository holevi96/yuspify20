...
var percentages = [3.0,2.5,2.0,1.5,1.0,1.0];
new Dragdealer('pricing-slider', {
    animationCallback: function(x, y) {
        var m = Math.round((x-(x % 0.25))/0.25); // melyik sávba esik a csúszka (0,1,2,3,4)
        var h = (x-m*0.25)/0.25; // az adott sávon belül hol helyezkedik el (0 és 1 közötti szám)
        var value = tomb[m]+h*(tomb[m+1]-tomb[m]);
        $('#monthly-revenue .revenue').text(Math.round(value*10));
        jQuery('.revenue-share').text(percentages[m]);
        jQuery('.yusp-generated-revenue').text(Math.round(value));
        jQuery('#the-price span').text(Math.round(value*0.01*percentages[m]));
    }
});