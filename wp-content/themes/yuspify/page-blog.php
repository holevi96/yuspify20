        ...
        <div class="yfy-container">
            <div id="blog-posts" class="col-xs-12">
                ...
                <div class="blog-first-row row">
                    ...
                    $exclude_array = array();
                    $row_type = get_field('first_row_choose','options');
                    if($row_type == "one"){
                        $p= get_field('one_column','options')[0];
                        echo get_field('latest_blogs_or_manual_choose');
                        if(get_field('latest_blogs_or_manual_choose','options') == 'latest'){

                            $p = get_posts(array('post_type'=>'post','numberposts'=>1))[0]->ID;
                        }
                        $exclude_array[] = $p;
                        setup_postdata($p);
                        ?>
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <?php
                                $p = get_field('two_column','options')[0];
                                if(get_field('latest_blogs_or_manual_choose','options') == 'latest'){
                                    $p = get_posts(array('post_type'=>'post','numberposts'=>2))[0]->ID;
                                }
                                $exclude_array[] = $p;
                                setup_postdata($p);
                                ?>
                            ...
                            <div class="col-lg-6">
                                <?php
                                wp_reset_postdata();
                                $p = get_field('two_column','options')[1];
                                if(get_field('latest_blogs_or_manual_choose','options') == 'latest'){
                                    $p = get_posts(array('post_type'=>'post','numberposts'=>2))[1]->ID;
                                }
                                $exclude_array[] = $p;
                                setup_postdata($p);
                                ?>
